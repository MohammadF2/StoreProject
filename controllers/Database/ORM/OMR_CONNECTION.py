...
import controllers.Database.ORM.Sql_Class
from controllers.Database.ORM.Sql_Class import Customer, Item, Order, CartItem, Cart
...
def get_customer_carts(customer_id):
    cart = session.query(controllers.Database.ORM.Sql_Class.Cart).filter(
        controllers.Database.ORM.Sql_Class.Cart.customer_id == customer_id).first()
    if cart:
...
def create_customer(name, phone):
    new_customer = controllers.Database.ORM.Sql_Class.Customer(name=name, phone=phone)
...
def get_or_create_customer_cart(customer_id):
    ...
    if cart:
        ...
        new_cart = controllers.Database.ORM.Sql_Class.Cart(customer_id=customer_id)
...
def get_customer_by_phone(phone):
    customer = session.query(controllers.Database.ORM.Sql_Class.Customer).filter(
        controllers.Database.ORM.Sql_Class.Customer.phone == phone).first()
    if customer:
...
def get_all_items():
    items_data = session.query(controllers.Database.ORM.Sql_Class.Item).all()
    items = []
...
def add_item_to_cart(cart_id, item_barcode, quantity):
    contains = session.query(controllers.Database.ORM.Sql_Class.CartItem).filter(controllers.Database.ORM.Sql_Class.CartItem.cart_id == cart_id,
                                                                    controllers.Database.ORM.Sql_Class.CartItem.item_barcode == item_barcode).first()
    if contains:
...
def update_item_quantity(item_barcode, quantity):
    item = session.query(controllers.Database.ORM.Sql_Class.Item).filter(controllers.Database.ORM.Sql_Class.Item.barcode == item_barcode).first()
    if item:
...
def get_orders(customer_id):
    orders = session.query(controllers.Database.ORM.Sql_Class.Order).filter(
        controllers.Database.ORM.Sql_Class.Order.customer_id == customer_id).all()
    orders_arr = []
    for order in orders:
        orders_arr.append(
...
def return_orders_as_json(customer_id):
...
def get_item_by_barcode(barcode):
    item = session.query(controllers.Database.ORM.Sql_Class.Item).filter(controllers.Database.ORM.Sql_Class.Item.barcode == barcode).first()
    return item


def get_all_cart_items(cart_id):
    results = session.query(controllers.Database.ORM.Sql_Class.CartItem).filter(
        controllers.Database.ORM.Sql_Class.CartItem.cart_id == cart_id).all()
    items = []
    for row in results:
...
def get_item_data(item_barcode):
    try:
        item = session.query(controllers.Database.ORM.Sql_Class.Item).filter(
            controllers.Database.ORM.Sql_Class.Item.barcode == item_barcode).first()
        if item:
...
def insert_order(cart_id, customer_id, total, discount):
    new_order = controllers.Database.ORM.Sql_Class.Order(cart_id=cart_id, customer_id=customer_id, total=total, discount=discount)
...
def get_max_order_no():
    order = session.query(controllers.Database.ORM.Sql_Class.Order).order_by(controllers.Database.ORM.Sql_Class.Order.order_No.desc()).first()
    if order:
...
def check_out_cart(cart_id, total, discount, customer_id):
    ...
    cart = session.query(controllers.Database.ORM.Sql_Class.Cart).filter(controllers.Database.ORM.Sql_Class.Cart.cart_id == cart_id).first()
    if cart:
        cart.customer_id = 1
        try:
            session.commit()
